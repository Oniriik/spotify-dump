      ...
      <div class="profile-user">
        ...
        <div class="btn-container">
          <div class="playlist-container profile-item">
            ...
            <button v-if="!isplaylistIsCreated" class="dl-btn" @click.prevent="this.createplaylist()" >Create</button>
            <button v-else class="dl-btn playlist"><a :href="playlistUrl" target="_blank"  >Open</a></button>
          </div>
          <div class="download-container center profile-item">
            ...
            <p class="ios ios2">
             /!\<br> We have some issues with the download :/ <br>
              Please screenshot instead, we will fix it asap ! 
            </p>
          </div>
        </div>
        <p class="logout" @click="this.state.dispatch('clearState')">Logout</p>
      </div>
      <div>
...
export default {
  ...
  data(){
    return{
      user: this.$store.state.user.data.images ? this.$store.state.user.data : null,
  ...
  methods:{
    ...
    downloadImg(){
      ...
      window.scrollTo(0,0)
      html2canvas(clone,{scrollX:-window.scrollX,scrollY:-window.scrollY}).then((canvas)=>{
    ...
    createplaylist(){
      ...
      axios.post(`https://api.spotify.com/v1/users/${this.$store.state.user.data.id}/playlists`, JSON.stringify({
            name: `My ${date} Dump`,
            description: "My monthly spotify dump profided by https://spotify-dump.xyz/",
            public: true
      }),{
        headers:{
        ...
        axios.post(`https://api.spotify.com/v1/playlists/${res.data.id}/tracks?uris=${encodeURI(tracks.join(","))}`,{},{
        headers:{
          "Accept": "application/json",
          "Content-Type": "application/json",
          "Authorization": "Bearer " + this.$store.state.user.token.access_token
